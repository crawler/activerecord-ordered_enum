...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/crawler/activerecord-ordered_enum/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("activerecord-ordered_enum.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in activerecord-ordered_enum.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "activerecord-ordered_enum.gemspec", /ruby_version = .*">= (.*)"/ => lowest
